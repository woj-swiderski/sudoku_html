...
function generateSelector(){
    // creates html structure and array for manipulation
    let temp = [];
...
function buildSudokuHTML(){
    // builds sudoku html table and fills in sudokuBase for easier manipulation
    for (let i = 0; i < 9; i++){
        let r = document.createElement("tr");
        sudokuTable.appendChild(r);
        for (let j = 0; j < 9; j++){
            let c = document.createElement("td");
            c.id = i + "" + j;      // <- is is not necessary, i don't make any use of it
            c.innerHTML = "";
...
function fillSudokuHTML(){
    for (let i = 0; i < 9; i++){
        for (let j = 0; j < 9; j++) {
            sudokuBase[9*i + j].value = sudoku[i][j];
            sudokuBase[9*i + j].locked = false;
            sudokuBase[9*i + j].style.backgroundColor = bgBackground;
            sudokuBase[9*i + j].innerHTML = "";
        }
    }
}
...
function finishSudoku(){
    for (let i = 0; i < 9; i++){
        for (let j = 0; j < 9; j++){
            if (sudokuBase[9*i + j].locked){
                sudokuBase[9*i + j].style.backgroundColor = bgLocked;
                sudokuBase[9*i + j].innerHTML = String(sudokuBase[9*i + j].value);
                selector[sudokuBase[9*i + j].value - 1].counter++;
            }
            else {
                sudoku[i][j] = 0;
            }
